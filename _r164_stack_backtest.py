#!/usr/bin/env python3
"""R164 — THE deploy-stack backtest number: blend k=0.5/0.6 + GATED_A1 on W2W3, both batches. Spec preds from r161 cache. VM ONLY.
...
for seeds, tag in ((SEEDS_STD, "std"), (SEEDS_ALT, "alt")):
    ...
    spec = pd.read_parquet(f"cache/r161_spec_venue5_{tag}_preds.parquet")
    nsc, pc = run_port(champ, f"champion {tag}")
    ...
    for k in (0.5, 0.6):
        ...
        nsb, pb = run_port(bl, f"blend k={k} {tag}")
        retb = ((1 + pb["net_ret"]).prod() - 1) * 100
        ddb = ((1 + pb["net_ret"]).cumprod() / (1 + pb["net_ret"]).cumprod().cummax() - 1).min() * 100
        print(f"     Ret={retb:+.1f}%  DD={ddb:+.1f}%  P={boot_paired(pb, pc):.3f}")
        nsg, pg = run_gated(bl, f"blend k={k} + GATED_A1 {tag}")
        retg = ((1 + pg["net_ret"]).prod() - 1) * 100
        ddg = ((1 + pg["net_ret"]).cumprod() / (1 + pg["net_ret"]).cumprod().cummax() - 1).min() * 100
        print(f"     STACK: Net={nsg:+.3f}  Ret={retg:+.1f}%  DD={ddg:+.1f}%  (vs champ {nsg-nsc:+.3f})")
print("R164 done.")
