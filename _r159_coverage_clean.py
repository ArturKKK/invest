#!/usr/bin/env python3
"""R159 (plan P2) — coverage-clean replication: WHOLE frame sliced to 2023-07+, W2+W3 only. Isolates the availability-artifact hypothesis. Original docstring:
 model-level test of the 5 STRONG cross-venue features. VM ONLY.
...
df = df[df["timestamp"] >= pd.Timestamp("2023-07-01", tz="UTC")].copy()
print(f"coverage-clean slice: {len(df):,} rows from {df['timestamp'].min()}")
from _research_r68_continuous_wf import CONTINUOUS_WINDOWS as _CW
CONTINUOUS_WINDOWS = _CW[1:]  # W2, W3 only
