#!/usr/bin/env python3
"""R166 — variance reduction: 10 seeds BOTH legs, stack k=0.5 + GATED_A1 on W2W3. VM ONLY.
...
SEEDS10 = SEEDS_STD + SEEDS_ALT
print("\n=== W2W3 10-SEED BOTH LEGS ===")
champ = train_ensemble(df, feats30, W23, seeds=SEEDS10,
                       cs_rank_exclude=[f for f in feats30 if f in MARKET_LEVEL_FEATURES])
spec = train_ensemble(df_spec, VENUE, W23, seeds=SEEDS10, cs_rank_exclude=[])
spec.to_parquet("cache/r166_spec_venue5_s10_preds.parquet", index=False)
nsc, pc = run_port(champ, "champion s10")
...
nsb, pb = run_port(bl, "blend k=0.5 s10")
print(f"   P={boot_paired(pb, pc):.3f}")
nsg, pg = run_gated(bl, "STACK k=0.5 + GATED_A1 s10")
retg = ((1 + pg["net_ret"]).prod() - 1) * 100
ddg = ((1 + pg["net_ret"]).cumprod() / (1 + pg["net_ret"]).cumprod().cummax() - 1).min() * 100
print(f"   STACK s10: Net={nsg:+.3f}  Ret={retg:+.1f}%  DD={ddg:+.1f}%  [refs: 5-seed stacks 2.741/2.795]")
print("R166 done.")
