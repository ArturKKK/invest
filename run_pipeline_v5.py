...
# Cost model for perpetual swaps (OKX, small capital $100-1K)
#   - Maker orders possible at this size (0.02% instead of 0.05%)
#   - Negligible slippage at sub-$1K
#   - Funding partially cancels in LS (longs pay, shorts receive)
#   - Real turnover is ~25% (most coins stay in quintile between rebalances)
COST_MODEL = {
    'taker_fee': 0.0003,        # 0.03% blended (mix of maker 0.02% + taker 0.05%)
    'slippage': 0.0001,         # 0.01% negligible at small size
    'funding_per_8h': 0.00005,  # 0.005% net (partially cancels in LS)
    'turnover_pct': 0.25,       # 25% realistic turnover per rebalance
}
...
def main():
    ...
    results_dir = args.results or os.path.join(project_root, 'results_v5')
    ...
    print("=" * 70)
    print("  CRYPTO ALPHA MODEL v5")
    print("  Sentiment + Risk Overlay + Rolling Walk-Forward")
    print("=" * 70)
    ...
    for w_idx, window in enumerate(windows):
        ...
        metrics, ls_net, ls_vt, ls_dd, timestamps = evaluate_model(
            test, 'pred_v5', target_col, HORIZON, label=window['name']
        )
        metrics['window'] = window['name']
        all_window_metrics.append(metrics)

        # Store test predictions and returns
        save_cols = ['timestamp', 'symbol', target_col, 'pred_v5']
    ...
    with open(os.path.join(results_dir, 'all_results_v5.json'), 'w') as f:
        json.dump(all_results, f, indent=2, default=str)

    if 'importance' in dir():
        importance.to_csv(os.path.join(results_dir, 'feature_importance_v5.csv'), index=False)
    ...
    if all_test_predictions:
        ...
        combined_preds.to_parquet(
            os.path.join(results_dir, 'test_predictions_v5.parquet'), index=False)
    ...
    eq.to_parquet(os.path.join(results_dir, 'equity_curves_v5.parquet'), index=False)
