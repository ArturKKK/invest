#!/usr/bin/env python3
"""R163 — FINAL validation: pristine blend on BOTH seed batches + GATED_A1 stack. VM ONLY.
...
for seeds, tag in ((SEEDS_STD, "std"), (SEEDS_ALT, "alt")):
    print(f"\n=== PRISTINE {tag.upper()} ===")
    champ_v3 = train_ensemble(df, feats30, [V3W], seeds=seeds,
                              cs_rank_exclude=[f for f in feats30 if f in MARKET_LEVEL_FEATURES])
    spec_v3 = train_ensemble(df_spec, VENUE, [V3W], seeds=seeds, cs_rank_exclude=[])
    nsc, pc = run_port(champ_v3, f"champion V3 {tag}")
    ...
    for k in (0.5, 0.6):
        ...
        print(f"     -> dSharpe={nsb-nsc:+.3f}  P={boot_paired(pb, pc):.3f}")
        nsg, pg = run_gated(bl, f"blend k={k} + GATED_A1 {tag}")
        print(f"     -> gated stack vs champ: {nsg-nsc:+.3f}")
print("R163 done.")
