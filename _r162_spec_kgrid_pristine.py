#!/usr/bin/env python3
"""R162 — specialist weight selection (W2W3, exploratory) + PRISTINE confirmation. VM ONLY.
...
with open("results_r162_kgrid.json", "w") as f:
    json.dump(results, f, indent=2, default=str)

# ── PART B: pristine confirmation (2026-04-26..06-08) ────────────────────
print("\n=== PART B: PRISTINE CONFIRMATION ===")
V3W = {"name": "V3", "train_end": "2026-02-25", "val_start": "2026-02-25",
       "val_end": "2026-04-25", "test_start": "2026-04-26", "test_end": "2026-06-08"}
champ_v3 = pd.read_parquet("cache/r143_V3_fresh_2026-02-25_30f_preds.parquet")
spec_v3 = train_ensemble(df_spec, VENUE, [V3W], seeds=SEEDS_STD, cs_rank_exclude=[])
spec_v3.to_parquet("cache/r162_spec_v3_preds.parquet", index=False)
ics = [spearmanr(g["pred"], g["fwd_ret"]).correlation
       for _, g in spec_v3.groupby("timestamp") if g["pred"].nunique() > 2]
ics = pd.Series([i for i in ics if not np.isnan(i)])
print(f"  spec V3 standalone pristine IC={ics.mean():+.4f} t_NW12={_nw_tstat(ics.values):+.2f} n={len(ics)}")
nsc, pc = run_port(champ_v3, "champion V3 pristine")
icc = [spearmanr(g["pred"], g["fwd_ret"]).correlation
       for _, g in champ_v3.groupby("timestamp") if g["pred"].nunique() > 2]
icc = pd.Series([i for i in icc if not np.isnan(i)])
print(f"  champ V3 pristine IC={icc.mean():+.4f} t_NW12={_nw_tstat(icc.values):+.2f}")
mg = champ_v3.merge(spec_v3[["timestamp", "symbol", "pred"]].rename(columns={"pred": "spred"}),
                    on=["timestamp", "symbol"], how="left")
mg["spred"] = mg["spred"].fillna(0.0)
for k in KGRID:
    bl = mg.copy(); bl["pred"] = bl["pred"] + k * bl["spred"]
    nsb, pb = run_port(bl, f"blend pristine k={k}")
    icb = [spearmanr(g["pred"], g["fwd_ret"]).correlation
           for _, g in bl.groupby("timestamp") if g["pred"].nunique() > 2]
    icb = pd.Series([i for i in icb if not np.isnan(i)])
    print(f"     IC={icb.mean():+.4f} t_NW12={_nw_tstat(icb.values):+.2f}  dSharpe={nsb-nsc:+.3f}  P={boot_paired(pb, pc):.3f}")
print("R162 done.")
