...
Результаты walk-forward (3 непересекающихся окна, ИСПРАВЛЕННЫЙ evaluate с реальными returns):

| Метрика | W1 (2024-H2) | W2 (2025-H1) | W3 (2025-H2+) | Avg |
|---------|-------------|-------------|---------------|-----|
| Rank_IC | 0.019 | 0.025 | 0.031 | 0.025 |
| ICIR | 0.12 | 0.43 | 0.30 | 0.28 |
| Rank_ICIR | 0.36 | 0.52 | 0.59 | 0.49 |
| LS_Sharpe_net | -2.21 | +1.95 | +0.42 | +0.05 |
| LS_MaxDD_net | -100% | -71% | -98% | -90% |
| DDStop_Sharpe | +0.06 | +3.09 | +0.92 | +1.36 |
| DDStop_MaxDD | -25% | -27% | -25% | -26% |

**Top features (gain):** btc_vol_24h, vol_12h_cs_rank, funding_rate_binance, btc_ret_12h, ret_12h, breadth_pct_positive, oi_change_24h, taker_cvd_24h

**Выводы:**
- Standalone: **очень слабая модель**. LS_Sharpe_net avg = 0.05 (практически ноль). W1 = катастрофа (-100% DD).
- Rank_IC = 0.025 — есть сигнал, но слабый (основная v7 даёт 0.024 avg по exp12)
- DDStop спасает (1.36 avg), но это за счёт "пережидания" 75% времени
- W2 сильно лучше W1/W3 — модель нестабильна по режимам

### Вопрос: стоит ли включать deriv_only в ансамбль?

При том что:
- standalone практически убыточна
- Rank_IC сопоставим с основной моделью (0.025 vs 0.024)
- но катастрофический MaxDD -100% без DDStop
- топ фичи = контекстные (btc_vol, vol_cs_rank), а не деривативные → модель скорее "market timing" чем "derivatives expert"

Варианты:
1. **Не включать** — слишком слабая, добавит шум в ансамбль
2. **Включить с пониженным весом** — как 4-й голос, но 0.5x от остальных
3. **Использовать как risk scaler** — не для selection, а для sizing (если deriv signal положительный → больше размер)
4. **Переработать** — убрать контекстные фичи, оставить только чистые деривативы
